 ...
 * (not headless Chrome, which is what the dottie-memory `browser_*` tools use).
 */
...
async function requireSafariRunning(toolName, input) {
  ...
  if (String(running).trim() !== 'true') {
    const msg = 'Safari is not running. Use mac_safari_open_url to open a page first.';
...
export const safariTools = tagDomain([
  createTool({
    ...
    description:
      ...
      'Do NOT use for Grok Hub / hub.grok.me / *.grok.me — use mac_hub_open (Dottie WebView). ' +
      'Use this to start any web flow — checking email in Gmail, opening an airline check-in page, looking up something on a site the user is already logged into. Prefer this over mac_open_url when the next step is to interact with the page. Then interact via mac_safari_click / mac_safari_fill (or mac_ax_tree / mac_ax_wait_for on com.apple.Safari — WebKit exposes the page\'s accessibility tree).',
    parameters: {
      type: 'object',
      properties: {
        url: {
    ...
    execute: async (input) => {
      ...
      if (!/^https?:\/\//i.test(url)) {
        const msg = 'mac_safari_open_url requires an absolute http(s) URL';
        logToolUse('mac_safari_open_url', input, msg);
        throw new Error(msg);
      }
  ...
  createTool({
    name: 'mac_safari_click',
    description:
      'Click an element on the web page open in Safari (link, button, form control) via the WebKit accessibility tree. Identify the element EITHER by `ref` (e.g. "@e5" from mac_ax_tree on com.apple.Safari) OR by `name` (its visible text, e.g. "Check In", "Sign In") — provide exactly one. Optionally pass `role` (e.g. "button", "link") to disambiguate a name. Use after mac_safari_open_url; use mac_ax_wait_for on com.apple.Safari to wait for the page to load. Destructive labels (Delete, Remove, etc.) ask for confirmation first.',
    parameters: {
      type: 'object',
      properties: {
        ref:  { type: 'string', description: 'Element ref like "@e5" from mac_ax_tree on com.apple.Safari. Provide ref OR name, not both.' },
    ...
    execute: async (input) => {
      try {
        await requireSafariRunning('mac_safari_click', input);
        ...
        logToolUse('mac_safari_click', input, msg);
        throw new Error(msg);
      }
    },
  }),
  createTool({
    name: 'mac_safari_fill',
    description:
      'Type a value into a form field on the web page open in Safari (search box, confirmation number, email field) via the WebKit accessibility tree. Identify the field EITHER by `ref` (from mac_ax_tree on com.apple.Safari) OR by `name` (its visible label or placeholder, e.g. "Confirmation number") — provide exactly one. Use after mac_safari_open_url; use mac_ax_wait_for on com.apple.Safari to wait for the page to load, then mac_safari_click to submit.',
    parameters: {
      type: 'object',
      properties: {
        ref:   { type: 'string', description: 'Element ref like "@e5" from mac_ax_tree on com.apple.Safari. Provide ref OR name, not both.' },
    ...
    execute: async (input) => {
      try {
        await requireSafariRunning('mac_safari_fill', input);
        ...
        logToolUse('mac_safari_fill', input, msg);
        throw new Error(msg);
      }
    },
  }),
  createTool({
    name: 'mac_safari_tabs',
